...
function game_add_random_tile( )
  empty_collect()
  if #EmptyCells.list == 0 then return end
  index = love.math.random(#EmptyCells.list)
  spot = EmptyCells.list[index]
  value = 2
  if love.math.random( ) > 0.9 then value = 4 end
...
function game_move_left()
  moved = false
  for row = 1, Game.rows do
    if line_apply_row_left(row) then
      moved = true
    end
  end
  return moved
...
function game_move_right()
  moved = false
  for row = 1, Game.rows do
    if line_apply_row_right(row) then
      moved = true
    end
  end
  return moved
...
function game_move_down( )
  moved = false
  for col = 1, Game.cols do
    if line_apply_col_down(col) then
      moved = true
    end
  end
  return moved
...
function game_can_merge_row( )
  for row = 1, Game.rows do
    for col = 1, Game.cols - 1 do
      if Game.cells[row][col] ~= 0 and
         Game.cells[row][col + 1] == Game.cells[row][col] then
        return true
      end
    end
  end
...
function game_can_merge_col( )
  for row = 1, Game.rows - 1 do
    for col = 1, Game.cols do
      if Game.cells[row][col] ~= 0 and
         Game.cells[row + 1][col] == Game.cells[row][col] then
        return true
      end
    end
  end
...
function game_can_merge( )
  if game_can_merge_row( ) then return true end
  if game_can_merge_col( ) then return true end
  return false
end
...
function draw_cell(row, col, value)
  ...
  if value == 0 then gfx.setColor(0.8, 0.8, 0.8)
  else gfx.setColor(1, 1, 0.7)
  end
  gfx.rectangle("fill", x, y, size, size)
  if value ~= 0 then gfx.setColor(0.2, 0.2, 0.2)
